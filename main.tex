...
Another cause of potential physical damage to workers is that the hammering task has to be carried out with: Yuan-Ho Chen and Yi-Lang Chen \cite{chenOptimalEffectiveWorking2018} studied the optimal height to realize this task and concluded should be between 25 cm and 35 cm below the elbow height. 
...
In our work, we will use the mc\_rtc open-source framework \cite{mcrtc} and its QP solver to develop the hammering task. The overall objective is to hammer a target (a nail in our case) with the maximum impact and mitigate the recoil damage on the robot's joints: 
this paper focuses on the impact maximization and the trajectory of the hammer tip to the nail. 
...
To realize a task, we compute a desired configuration acceleration $\mathbf{\ddot q}_d$, then we track how well the robot realizes this task using a cost function expressed as a function of the actual configuration acceleration $\mathbf{\ddot q}$.
...
\end{equation}
Taking $\arg \max_{\mathbf{\ddot q}}$ on both sides, the two first terms of the right hand side of the equation are discarded because they do not depend on $\mathbf{\ddot q}$ by (\ref{effective mass}) and (\ref{time derivative of the effective mass}) :
\begin{equation}
...
\noindent
The posture task is a task where the robot has to maintain a given configuration $\mathbf{q}$, the paper from Cisneros et al. \cite{cisnerosRobustHumanoidControl2018} gives a deeper look into it. 
...
\begin{figure}
    \centering
    \includesvg[width=0.8\linewidth]{Projected_momentum}
