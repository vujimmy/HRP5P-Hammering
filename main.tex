...
The task of hammering a nail is the topic of this article with the goal of hitting a nail and driving it into a wooden surface. 
...
then we present simulations using the mc\_rtc framework \cite{mcrtc} in Section \ref{Simulations} and we conclude our paper in Section \ref{Conclusion}.
...
The problem with this model is that the discontinuous velocity of the hammer tip at the instant of collision, $t_c$, as the time derivative at this point in time does not exist. Therefore, we can not use a force-based model.
\\
...
\end{equation}
Taking $\arg \max_{\mathbf{\ddot q}}$ on both sides, the two first terms of the right hand side of the equation are discarded because they do not depend on $\mathbf{\ddot q}$, then  by (\ref{effective mass}) and (\ref{time derivative of the effective mass}) we have:
\begin{equation}
...
These are all defined in the sense of Vaillant et al.
...
\\
This approach allows us to maximize the EMR online instead of optimizing offline. Using an offline method implies knowing the pose of the hitting target in advance every time, and this would not work if we were to change the pose of the nail. 
...
Simulations were done using mc\_mujoco \cite{singhMcmujocoSimulatingArticulated2022} and mc\_rtc with its QP solver in torque control mode, and a timestep $\Delta t = 0.002$s 
% and dynamic constraints set to $\bm \kappa_{\xi} = (0.1, 0.01, 0.5)^\mathbf{T}$
. 
The floating base of the robot is fixed as keeping balance after the impact is out of the scape of this paper. 
The hammer used weighs 0.792 kg, has a length of 328 mm and its contact surface is circular with radius 15.5 mm. 
The nail is fixed and "floating" in the environment to avoid earlier or post-impact collisions with an object (such as a table), and it has a length of 88.03 mm and its head has a radius of 6.95 mm. 
...
Then, we proposed our method to maximize the EMR using its gradient and QP, then we suggested an end-effector trajectory built from a constrained $5^{th}$ order Bézier curve. 
...
In the future, we want to work on the stability problem of the robot potentially induced by the post-impact rebounce of the hammer, minimizing the damage that the robot must endure on its joints and detection of the nail via computer vision. 
\label{Conclusion}
