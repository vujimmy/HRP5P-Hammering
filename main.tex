...
The task of hammering a nail is the topic of this article with the goal of hitting the nail and driving it into a wooden surface. 
...
Our linear constraints for the QP solver are described as i) joint limits, ii) joint velocity limits, iii) joint acceleration limits, iv) joint torque limits, v) dynamic constraints :
\begin{equation}
...
For the dynamic constraints (\ref{Dynamic constraints}), $d_i$ is the interaction distance, $d_s$ is the security distance and $\xi_{offset}$ is a fixed offset which allows the velocity damper to accelerate to avoid over-constraining the problem 
\cite{vaillantMulticontactVerticalLadder2016}
.
...
\label{Trick}
\end{equation}
subject to (\ref{Joint limits}), (\ref{Joint velocity limits}), (\ref{Joint acceleration limits}), (\ref{Torque limits}) and (\ref{Dynamic constraints}).
...
Simulations were done using mc\_mujoco and mc\_rtc in torque control mode with a timestep $\Delta t = 0.002$s and dynamic constraints set to $\bm \kappa_{\xi} = (0.1, 0.01, 0.5)^\mathbf{T}$. 
