...
\author{%
    ...
    \{rafael.cisneros, mehdi.benallegue\}@aist.go.jp}, 
    %
	%
}
...
Moreover, we implement a 5\textsuperscript{th} order Bézier curve as the trajectory to attain the nail at any reachable pose.
\end{abstract}
...
Inadequate working posture and repetitive movements have been identified to contribute to musculoskeletal disorders \cite{coteDifferencesMultijointKinematic2005}, which is the case for a hammering task. 
...
Indeed, as a result from \cite{petricHammeringDoesNot2017} indicates, hammers themselves are one of the non-powered handtools that cause the most injury \cite{aghazadehInjuriesDueHandtools1987}, especially to the fingers which are the most injured body part. 
Another cause of potential physical damage to workers is that the hammering task has to be carried out with: \cite{chenOptimalEffectiveWorking2018} studied the optimal height to realize this task and concluded should be between 25 cm and 35 cm below the elbow height. 
...
Other impact tasks have been extensively studied, Cisneros, et al. focused on impulsive pedipulation \cite{cisnerosImpulsivePedipulationSpherical2016} on a spherical object with a humanoid robot,
...
Tassi, et al. \cite{tassiIMAcatcherIMpactawareNonprehensile2025} developed a method using velocity matching and a motion planner to catch high-impact flying objects to reduce hardware damage to the robot. 
Furthermore, Wang and Kheddar \cite{wangImpactFriendlyRobustControl2019} worked on robot damage mitigation by designing a conservative, robust and impact-friendly robust controller using QP.
For our work, we will use the mc\_rtc open-source framework \cite{mcrtc} and its QP solver to develop the hammering task.
...
then we present simulations in Section \ref{Simulations} and finally we conclude our paper in Section \ref{Conclusion}.
...
where $\mathbf{q, \dot q, \ddot q} \in \mathbb{R}^n$ are the configuration, configuration velocity and configuration acceleration of the robot, respectively, $\mathbf{M(q)} \in \mathbb{R}^{n \times n}$ is the symmetric mass matrix or inertia matrix of the robot,
     $\mathbf{C(q, \dot q)} \in \mathbb{R}^{n \times n}$ is a factorization of the Coriolis matrix \cite{bjerkengNewCoriolisMatrix2012},
...
\label{map xdot qdot}
\end{equation}
where $\mathbf{\dot x} \in \mathbb{R}^6$ is the task space velocity (translational and angular velocities) of the end-effector and the time derivative of the end-effector pose $\mathbf{x} \in \mathbb{R}^6$ :
\begin{equation}
...
\noindent
From Stronge \cite{STRONGE}, we define the common tangent plane of collision as the plane passing through the contact point at the instant of collision that is tangent to both surfaces involved in the collision and the normal unit vector to the collision $\mathbf{n} \in \mathbb{R}^3$ as a vector orthogonal to that plane, see the schematic on Fig. \ref{SCHEMATIC OF THE PROBLEM}.
...
\noindent
Let us define $m_{e,\mathbf{n}}(\mathbf{q}) \in \mathbb{R^+}$ (the "e" stands for "effective"), the configuration dependant Effective Mass of the Robot (EMR), as :
\begin{equation}
    m_{e,\mathbf{n}}(\mathbf{q}) \triangleq \left(\mathbf{n^T} \bm \Lambda (\mathbf{q}) \mathbf{n} \right)^{-1}
\label{effective mass}
\end{equation}
with $\bm \Lambda \in \mathbb{R}^{3 \times 3}$ being the mobility matrix defined as :
\begin{equation}
...
The problem with this model is that the discontinuous velocity of the hammer tip at the instant of collision, $t_c$, as the time derivative at this point in time does not exist. We can not use a force-based model.
\\
\subsubsection{Energy-based model}
\noindent
\label{Energy-based model}
An energy-based model could be used to overcome the problem of the discontinuity of the hammer tip velocity. 
...
Thus, although the model is still usable, we would either have to estimate the losses or neglect them making the model less precise.
\\
...
With momentum, the microscopic interactions between the hammer tip and the nail are taken into account through a scalar number $e_*$ called \textit{coefficient of restitution}, which measures the elasticity of the collision. 
...
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{Schematic of the problem.png}
...
At the instant of contact $t_c$, these points collide and thus share the same coordinates (neglecting the radius of the infinitesimal particle, see Fig. \ref{SCHEMATIC OF THE PROBLEM}) :
\begin{equation}
...
\end{equation}
The velocity and acceleration of the hammer tip is given by the trajectory that will be introduced in Section \ref{Hammer tip velocity and acceleration}. \\
...
\label{impulse}
\end{equation}
To maximize the impact, we want to maximize the impulse given by (\ref{impulse}) which is equivalent to maximizing the projected momentum $P(t) \in \mathbb{R}$ of the hammer tip along the normal $\mathbf{n}$ defined as :
\begin{equation}
...
\end{equation}
Let's call this objective function the Effective Mass Maximization Task (EMMT). 
The goal of this task is to increase as much as possible the EMR at the next iteration of the controller without its value dropping down. 
However, we cannot assert that the result will be a global maximum because our problem is not convex. 
...
For the dynamic constraints (\ref{Dynamic constraints}), $d_i$ is the interaction distance, $d_s$ is the security distance and $\xi_{offset}$ is a fixed offset, all developed in Vaillant, et al.
...
Indeed, we want to align three vectors: the vector normal to the contact surface of the hammer tip which we will call $\mathbf{n}^h \in \mathbb{R}^3$ (expressed in the hammer tip frame), the $\mathbf{n}$ vector and finally the vector between the hammer tip and the contact point of the nail. 
...
\begin{equation}
     A_{VOT}(\mathbf{\ddot q}) = \| K_{p, VOT} \bm \epsilon + K_{d,VOT} \bm {\dot \epsilon}- \mathbf{\ddot x}_{\omega}(\mathbf{\ddot q}) \|^2 
\end{equation}
where $K_{p, VOT}$, $K_{d, VOT} \in \mathbb{R}^+$ are its stiffness and damping, respectively. 
\begin{figure}
    \centering
    \includesvg[width=1\linewidth]{EMMT 1_1}
...
\begin{figure}
    \centering
    \includesvg[width=1\linewidth]{Projected_momentum}
...
\begin{figure*}
    \vspace*{-2.3cm}
    \centering
    \includesvg[width=0.8\linewidth]{Velocities}
    \caption{Velocity tracking of the hammer tip for different $K_{p, VOT}$ values}
    \label{Velocity tracking of the hammer tip}
\end{figure*}

\noindent
In this section, we present the simulations made on our humanoid robot HRP-5P. The setting is a fixed hammer in one of the robots' grippers (the left one in our case), a fully known position and orientation of the nail and a prefixed desired hitting velocity of the hammer tip on the nail.  
Simulations were done using mc\_mujoco and mc\_rtc \cite{singhMcmujocoSimulatingArticulated2022} in torque control mode with a timestep $\Delta t = 0.002$s 
...
Then, we proposed our method to maximize the EMR using QP and its gradient and we suggested a trajectory constructed from a constrained $5^{th}$ order Bézier curve. 
...
Possible improvements include an estimation of the nail penetration in the wood along with a simulation of the dynamics of the nail, a way to build our path and orientation to the nail by efficiently considering collision constraints with the environment or a post-impact that utilizes the post-impact velocity of the hammer tip to prepare for the next hit, as the robot only goes back to its initial posture for now. 
In the future, we want to work on the stability problem of the robot potentially induced by the post-impact rebounce of the hammer, minimizing the damage that the robot must endure on its joints during the hit to avoid damage as much as possible and detection of the nail via computer vision. 
\label{Conclusion}
