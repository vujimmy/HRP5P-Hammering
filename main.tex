...
\author{%
    ...
    $^{*}$ Corresponding author. $^{\#}$ Versailles University.
    }
    %
	%
}
...
\end{equation}
For a fixed given target normal velocity $\mathbf{\dot x_\nu^T n}$ (the velocity we want the hammer tip to reach at the nail), we see that in order to maximize the projected momentum of the hammer tip, we have to maximize the EMR. This procedure is explained in Section \ref{Optimization of the effective mass using QP}.
...
\end{equation}
This objective function is the Effective Mass Maximization Task (EMMT). 
...
\noindent
We call this task the Vector Orientation Task (VOT).
...
\begin{figure*}
    % \vspace*{-2.3cm}
    \centering
    \includesvg[width=\linewidth]{Figure_1.svg}
...
Simulations were done using mc\_mujoco and mc\_rtc \cite{singhMcmujocoSimulatingArticulated2022} with its QP solver in torque control mode, and a timestep $\Delta t = 0.002$s, see Fig.~\ref{Snapshots}. 
...
We set $T = 1$s, $W_{B} = 100$, $W_{VOT} = 10$, $W_{m} = 100$, $W_{p} = 1$, ${}^{nail}\mathbf{\dot x}_{\nu}^{t_c} = (0, 0 ,  -2.0) ^\mathbf{T}$ m/s, the stiffness of the BSTT to $K_{p, B} = 100000$ and the stiffness of the VOT to $K_{p, VOT} = 250$ with their damping given by (\ref{Kp Kd relationship}). 
We see the tracking of the hammer tip velocity in Fig.~\ref{Velocity tracking of the hammer tip}(a) and (b). 
It is worth to mention that the hammer tip properly reaches ${}^{nail}\mathbf{\dot x}_{\nu}^{t_c}$.
% until the sudden spike in velocity at the end, which is due to a collision with the nail. 
For $K_{p, VOT} = 250$, we record an orientation error of $11.88$ deg and some oscillations at the beginning. 
The stiffness of the VOT is decreased to 100 in Fig.~\ref{Velocity tracking of the hammer tip}(b), which removed the oscillations. 
%However, we also see some unwanted oscillations on the $z$ coordinate: this is due to the higher stiffness ($250$) of the VOT. 
We also tested $K_{p, VOT} = 1000$: the final orientation error has been recorded to be $4.16$ deg, however we observed large oscillations in the first instants. 
...
\begin{figure}
    \centering
    \includegraphics[width=8.2cm, height=4cm]{Sim snapshot 2.png}
    \caption{Snapshots of the simulation in mc\_mujoco}
    \label{Snapshots}
\end{figure}
\section{Conclusion}
\noindent
...
In the future, we want to work on the stability problem of the robot potentially induced by the post-impact rebounce of the hammer, damage minimization on the robot's joints, feasibility of the hammering task on the free-standing robot and detection of the nail via computer vision. 
\label{Conclusion}
