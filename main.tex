...
\end{equation}
where $\mathbf{\dot x_{\nu}} \in \mathbb{R}^3$ is the translational part of $\mathbf{\dot x}$ defined in (\ref{map xdot qdot}), $\mathbf{F}^h \in \mathbb{R}^3$ is the force applied to the nail by the hammer (the superscript "h" stands for "hammer") and $m_{e,\mathbf{n}}$ is the Effective Mass of the Robot (EMR) at the point of impact.
...
\label{PD controller}
\end{equation}
where $K_p$ and $K_d$ are the proportional (stiffness) and derivative (damping) gains of the posture task, respectively, $\mathbf{q_d}$, $\mathbf{\dot q_d}$ and $\mathbf{\ddot q_d} \in \mathbb{R}^n$ are the desired configuration, desired configuration velocity and desired configuration acceleration, respectively.\\
...
Let $\epsilon_{\mathbf{x}_\nu} = \mathbf{x}_\nu - \mathbf{x}_{\nu, d}$ where $\mathbf{x}_{\nu, d}$ is the desired end-effector position given by $\bm \Gamma$. The cost function of this task, $A_{B}(\mathbf{\ddot q})$, is :
{\small
\begin{equation}
    A_{B}(\mathbf{\ddot q}) = W_{B}\| K_{p, B} \bm \epsilon_{\mathbf{x}_\nu}+ K_{d, B} \bm {\dot \epsilon_{\mathbf{x}_\nu}} + \mathbf{\ddot x}_{\nu, d} - \mathbf{\dot J_\nu \dot q} - \mathbf{J_\nu \ddot q} \|^2
\end{equation}
}
where $W_B$ is the relative weight associated to the BSTT and $K_{p, B}$, $K_{d, B} \in \mathbb{R}^+$ are its stiffness and damping, respectively.
...
\begin{figure}
    \centering
    \includesvg[width=0.75\linewidth]{Projected_momentum}
...
Then, we proposed our method to maximize the EMR using its gradient and QP and we suggested an end-effector trajectory built from a constrained $5^{th}$ order Bézier curve. 
